...
# 计算差异 (differ = action_sums - args_sums)
differ = action_sums - args_sums
...
differ_scaled = scaler.fit_transform(differ.reshape(-1, 1)).flatten()

# 确定top 20%重要性层（基于各自的总和）
...
# Action总和
ax1.set_title("Sum of Action Layer Importance", fontsize=14, fontweight="bold")
for i, layer in enumerate(layers):
    ...
    # 添加原始值标签
    # ax1.text(layer, action_sums_scaled[i] + (0.05 if action_sums_scaled[i] >= 0 else -0.1),
    #          f"{action_sums[i]:.1f}", ha="center", fontsize=8)
...
# Args总和
ax2.set_title("Sum of Args Layer Importance", fontsize=14, fontweight="bold")
for i, layer in enumerate(layers):
    ...
    # 添加原始值标签
    # ax2.text(layer, args_sums_scaled[i] + (0.05 if args_sums_scaled[i] >= 0 else -0.1),
    #          f"{args_sums[i]:.1f}", ha="center", fontsize=8)
...
# 差异图 (Action - Args)
ax3.set_title("Difference Between Action and Args Importance (Action - Args)", 
              fontsize=14, fontweight="bold")
for i, layer in enumerate(layers):
    color = colors["differ"]
    ax3.bar(layer, differ_scaled[i], color=color, alpha=0.8,
            edgecolor="black", linewidth=0.7, width=bar_width)
    
    # 添加原始差异值标签
    # ax3.text(layer, differ_scaled[i] + (0.05 if differ_scaled[i] >= 0 else -0.1),
    #          f"{differ[i]:.1f}", ha="center", fontsize=8)

ax3.set_ylim(-1.2, 1.2)
ax3.set_yticks([-1, -0.5, 0, 0.5, 1])
ax3.set_xlabel("Layer Index (0-31)", fontsize=12)
ax3.set_ylabel("Scaled Difference", fontsize=12)
...
table_data.append([
    "Action", 
    ", ".join(map(str, sorted(top_action_layers))),
    f"{top_action_avg:.2f}"
])
...
table_data.append([
    "Args", 
    ", ".join(map(str, sorted(top_args_layers))),
    f"{top_args_avg:.2f}"
])
...
table_data.append([
    "Difference", 
    ", ".join(map(str, sorted(top_differ_layers))),
    f"{top_differ_avg:.2f}"
])
...
# 保存表格
table_save_path = os.path.join(save_dir, "top_layers_comparison.png")
