...
CUDA_VISIBLE_DEVICES_STR="${CUDA_VISIBLE_DEVICES-}"
echo "CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES_STR}"
if [[ -z "${CUDA_VISIBLE_DEVICES_STR}" ]]; then
  echo "ERROR: CUDA_VISIBLE_DEVICES is empty"
  exit 1
fi
CVD_CLEAN="${CUDA_VISIBLE_DEVICES_STR// /}"
NGPUS=0
IFS=',' read -r -a _CVD_LIST <<< "$CVD_CLEAN"
for d in "${_CVD_LIST[@]}"; do
  if [[ -n "$d" ]]; then
    ((NGPUS+=1))
  fi
done
if (( NGPUS <= 0 )); then
  echo "ERROR: failed to infer GPU count from CUDA_VISIBLE_DEVICES='${CUDA_VISIBLE_DEVICES_STR}'"
  exit 1
fi
