...
class AgentLoopWorkerBase:
    ...
    def _postprocess(self, inputs: list[_InternalAgentLoopOutput]) -> DataProto:
        ...
        # FIXME 这里是reward tensor => scores的分数是bs*1
        print("reward scores:", scores)
        use_last_token_only = False
        ...
        if all(score is not None for score in scores):
            ...
            for i, input in enumerate(inputs):
                ...
                reward_extra_info = input.extra_fields.get("reward_extra_info", {})
                process_reward = reward_extra_info.get("process_reward", [])
                if process_reward:
                    if isinstance(process_reward, dict):
                        process_reward = [process_reward]
                    for slice in process_reward:
                        from_idx = slice.get("from_idx", 0)
                        to_idx = slice.get("to_idx", from_idx)
                        score_value = float(slice.get("score_value", 0.0))
                        # 防止越界
                        from_idx = min(max(from_idx, 0), rm_scores.size(1)-1)
                        to_idx = min(max(to_idx, 0), rm_scores.size(1)-1)
                        span_len = max(1, to_idx - from_idx + 1)
                        if not use_last_token_only:
                            # 平均分配
                            per_token = score_value / span_len
                            rm_scores[i, from_idx:to_idx+1] += per_token
                        else:
                            # 只给最后一个 token
                            rm_scores[i, to_idx] += score_value
